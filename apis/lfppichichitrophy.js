            ...
            dbd.find({}).skip(offset).limit(limit).toArray(function(err, lfppichichitrophy) {
                if (err) {
                    ...
                    if (from && to) {
                            ...
                            aux = buscador(lfppichichitrophy, aux, from, to);
                            if (aux.length > 0) {
                                aux2 = aux.slice(offset, offset+limit);
                                console.log("INFO: Sending results with from and to and limit and offset: " + JSON.stringify(aux, 2, null));
                                console.log("INFO: Sending results with from and to and limit and offset: " + JSON.stringify(lfppichichitrophy, 2, null));
...
var buscador = function(base, conjuntoauxiliar, desde, hasta) {
    ...
    for (var j = 0; j < base.length; j++) {
        var season = base[j].season;
        if (to >= season && from <= season) {
